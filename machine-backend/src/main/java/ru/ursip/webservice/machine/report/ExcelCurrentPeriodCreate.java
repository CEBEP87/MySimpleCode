...
import ru.XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX.webservice.machine.service.impl.CalculationsServiceImpl;
 ...
 * @since 07.11.2017
 */
public class ExcelCurrentPeriodCreate {
     ...
     * @return - excel отчет
     */
    public static Workbook createNewExcelDocument(Period period) throws IOException {
        resultExcel = new SXSSFWorkbook();

        settings = ReportConfiguration.getReportSettings(ReportDocTypeProperties.report_current_period);
        ...
        String currentPeriodStringName = period.getFields().getPName() + " " + currentPeriodCalendar.get(Calendar.YEAR) + " год";
        ...
        if (settings.getTableHeadColumns() != null) {
            ...
            for (Resource resourcePeriod : period.getResource()) {
                Row resourcePeriodRow = createTableRow(++rowNum, resourcePeriod, ++countResourcePeriod);
                //Установка стилей для строки информации о ресурсы в периоде
     ...
     */
    private static Row createTableRow(int rowNum, Resource currentResourcePeriod, int countResourcePeriod) {
        ...
        if (ObjectUtils.notEqual(currentResourcePeriod.getFields().getRName(), null)) {
            infoRow.getCell(2).setCellValue(currentResourcePeriod.getId());
        }
